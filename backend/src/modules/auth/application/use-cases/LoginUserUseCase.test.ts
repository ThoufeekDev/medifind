...
describe("LoginUserUseCase", () => {
    ...
    it("should throw an error when user password is wrong", async () => {
        ...
        await expect(loginUserUseCase.execute({ email: "john@example.com", password: "wrong-password", role: Role.USER }))
    ...
    it("should throw ForbiddenError for unverified user", async () => {
        ...
        await expect(loginResponse.execute({ email: "john@example.com", password: "hashed-password", role: Role.USER }))
    ...
    it("should throw UnauthorizedError when user role does not match", async () => {
        ...
        vi.mocked(comparePassword)
            .mockResolvedValue(true)
        const loginResponse = new LoginUserUseCase(userRepository)
        await expect(loginResponse.execute({ email: "john@example.com", password: "hashed-password", role: Role.ADMIN }))
            .rejects.toThrow(UnauthorizedError)
    ...
    it("should login succesfully with valid credentials",async()=>{
        ...
        const response =  await loginUserUseCase.execute({email:"john@example.com",password:"hashed-password",role:Role.USER})
